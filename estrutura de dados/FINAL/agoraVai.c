...
int lerDadosCSV(const char *nomeArquivo, RegistroCriminalidade ***registros, int *numRegistros, HashTable *tabelaHash) {
    ...
    while (fgets(linha, sizeof(linha), fp)) {
        ...
        // Definir o mês para o registro
        int i;
        registro->mes = i; // Supondo que 'i' é o mês atual do loop
...
RegistroCriminalidade *buscarRegistroComBSearchPorNomeEMes(RegistroCriminalidade **registros, int numRegistros, const char *nome_cidade, int mes, int *comparacoes) {
    RegistroCriminalidade chave;
    strcpy(chave.nome_cidade, nome_cidade);
    chave.mes = mes;

    *comparacoes = 0;
    RegistroCriminalidade *resultado = bsearch(&chave, registros, numRegistros, sizeof(RegistroCriminalidade *), comparaRegistrosPorNomeEMes);
...
RegistroCriminalidade *buscarRegistroComBSearchPorNomeEMes(RegistroCriminalidade **registros, int numRegistros, const char *nome_cidade, int mes, int *comparacoes) {
    ...
    *comparacoes = 0;
    ...
    while (esquerda <= direita) {
        (*comparacoes)++;
        int meio = (esquerda + direita) / 2;
        int resultadoComparacaoNome = strcmp(registros[meio]->nome_cidade, nome_cidade);
        int resultadoComparacaoMes = registros[meio]->mes - mes;
        
        if (resultadoComparacaoNome == 0 && resultadoComparacaoMes == 0) {
            return registros[meio];
        } else if (resultadoComparacaoNome < 0 || (resultadoComparacaoNome == 0 && resultadoComparacaoMes < 0)) {
            esquerda = meio + 1;
        } else {
            direita = meio - 1;
        }
...
// Função para buscar um registro com bsearch por nome e mês
void buscarPorNomeEMesComBSearch(RegistroCriminalidade **registros, int numRegistros) {
    char nome_cidade[100];
    int mes;
    int comparacoes = 0;
    ...
    // Ordenar os registros por nome da cidade e mês usando qsort (já feito no main)
    
    // Buscar o registro utilizando bsearch
    RegistroCriminalidade *registroEncontrado = buscarRegistroComBSearchPorNomeEMes(registros, numRegistros, nome_cidade, mes, &comparacoes);
    
    if (registroEncontrado != NULL) {
        mostrarRegistro(registroEncontrado);
        printf("Comparacoes necessarias: %d\n", comparacoes);
    } else {
        printf("Registro para cidade %s e mês %d não encontrado.\n", nome_cidade, mes);
    }
}
